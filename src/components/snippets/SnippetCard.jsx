...
const BASE =
  'w-full h-[100px] rounded-[10px] border transition-[border-color,background-color] duration-150'
const ACTIVE =
  'border-[color-mix(in_srgb,var(--accent-try)_60%,var(--header-border))] bg-[color-mix(in_srgb,var(--accent-try)_8%,var(--app-bg))]'
...
export default function SnippetCard({
  ...
  if (onDelete) {
    return (
      <div
        className={`relative flex flex-row items-stretch overflow-hidden ${BASE} ${active ? ACTIVE : `${INACTIVE} hover:border-app-muted hover:bg-app-btn-hover`}`}
      >
        <button
