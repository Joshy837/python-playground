...
export default function CourseNode({ data }) {
  ...
  }, [expanded, popupExiting, steps.length])
  ...
  return (
    ...
    >
      ...
      />
      <div
        className={`h-[13px] flex items-center ${done ? 'text-app-green' : 'text-app-muted'}`}
      >
        {done ? (
          <Check size={13} />
        ) : !unlocked ? (
      ...
      <span
        className={`text-[0.75rem] font-semibold ${unlocked || done ? 'text-app-fg' : 'text-app-muted'}`}
      >
        {title}
      </span>
        ...
        >
          {steps.map((step, i) => (
            <StepButton
              ...
              title={step.title}
              style={{
                animationDelay: popupExiting
                  ? `${(steps.length - 1 - i) * 30}ms`
                  : `${i * 60}ms`,
              }}
              onClick={(e) => {
                e.stopPropagation()
