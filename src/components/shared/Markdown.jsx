      ...
      tokens.length === 0
        ...
        : tokens.map((tok, i) => {
            const text = line.slice(
              tok.offset,
              tokens[i + 1]?.offset ?? line.length
            )
            const [color, italic] = resolveTokenColor(tok.type, colors)
            return color ? (
              <span
                key={`${li}-${i}`}
                style={{ color, ...(italic && { fontStyle: 'italic' }) }}
              >
                {text}
              </span>
            ) : (
...
const MD_CLS = {
  ...
  inlineCode:
    'font-mono text-[0.82em] py-[0.12em] px-[0.4em] rounded-[4px] bg-app-muted/15',
  codeBlock:
    'bg-[var(--header-bg)] border border-[var(--header-border)] rounded-[8px] py-[0.9rem] px-[1.1rem] font-mono text-[0.82rem] leading-[1.65] overflow-x-auto mt-[0.75rem] mb-4 text-[var(--output-stdout)]',
}
...
function parseInline(text) {
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    if (m[0].startsWith('**')) {
      ...
      parts.push(
        <code key={k++} className={MD_CLS.inlineCode}>
          {m[2]}
        </code>
      )
    }
...
function renderBlock(block, key) {
  ...
  if (h1m)
    return (
      <h1 key={key} className={MD_CLS.h1}>
        {parseInline(h1m[1])}
      </h1>
    )
  ...
  if (h2m)
    return (
      <h2 key={key} className={MD_CLS.h2}>
        {parseInline(h2m[1])}
      </h2>
    )
