    ...
    <div className="lesson-section--challenge lesson-section-pop">
      ...
      <div className="border border-app-border rounded-[10px] overflow-hidden mt-4">
        ...
        <div className="flex flex-row items-stretch">
          <div
            ref={containerRef}
            className="lesson-editor-pane flex-1 min-w-0"
          />
          <div className="w-[40%] border-l border-app-output-border bg-app-surface px-[0.9rem] py-[0.65rem] overflow-y-auto">
            ...
            runtimeOutput?.stdout ? (
              <>
                {runtimeOutput.error && (
                  <pre className="text-app-error text-xs whitespace-pre-wrap">
                    {runtimeOutput.error}
                  </pre>
                )}
                {runtimeOutput.stderr && (
                  <pre className="text-app-stderr text-xs whitespace-pre-wrap">
                    {runtimeOutput.stderr}
                  </pre>
                )}
                {runtimeOutput.stdout && (
                  <pre className="text-app-stdout text-xs whitespace-pre-wrap">
                    {runtimeOutput.stdout}
                  </pre>
                )}
              </>
      ...
      {testResults && (
        <div className="flex flex-col gap-2 mt-4">
          {testResults.map((t, i) => (
            <div
              key={i}
              className="flex items-start gap-[0.6rem] text-[0.84rem]"
            >
              <span
                className="shrink-0"
                style={{
                  color: t.passed ? 'var(--status-green)' : 'var(--status-red)',
                }}
              >
                {t.passed ? '✓' : '✗'}
              </span>
              <span
                className={`text-sm ${t.passed ? 'text-app-muted' : 'text-app-fg'}`}
              >
                {t.name}
                {!t.passed && t.error && (
                  <span className="text-app-error block text-xs mt-0.5">
                    {t.error}
                  </span>
                )}
              </span>
      ...
      {allPassed && (
        <div className="flex items-center justify-between gap-4 mt-6 py-4 px-5 rounded-[10px] bg-[color-mix(in_srgb,var(--status-green)_10%,var(--header-bg))] border border-app-green/30">
          ...
          {isLastStep ? (
            <button
              className="bg-app-btn text-app-fg transition-colors duration-150 hover:bg-app-btn-hover text-sm px-4 py-1.5 rounded-lg"
            ...
            <button
              className="inline-flex items-center gap-[4px] py-[0.35rem] px-[0.85rem] rounded-[8px] border-none bg-app-green text-black text-[0.82rem] font-semibold cursor-pointer transition-opacity duration-150 hover:opacity-85"
              onClick={onNextStep}
            >
