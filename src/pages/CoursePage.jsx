...
export default function CoursePage() {
  ...
  const nodes = useMemo(
    () =>
      NODES.map((node) => {
        ...
        const isNodeUnlocked = node.requires.every((req) =>
        ...
        return {
          ...
          data: {
            ...
            onStepClick: (stepIdx) =>
              navigate(`/learn/${node.id}/${stepIdx + 1}`),
          },
        }
      }),
    [completed, expandedId, isMobile, isStepComplete, isStepUnlocked, navigate]
  )
  ...
  const edges = useMemo(
    () =>
      EDGES.map(({ from, to }) => {
        ...
        const unlocked = NODES.find((n) => n.id === to)?.requires.every((req) =>
          completed.includes(req)
        )
        return {
          id: `${from}-${to}`,
  ...
  const translateExtent = useMemo(() => {
    ...
    return [
      [
        Math.min(...xs) - NODE_W / 2 - PADDING,
        Math.min(...ys) - NODE_H / 2 - PADDING,
      ],
      [
        Math.max(...xs) + NODE_W / 2 + PADDING,
        Math.max(...ys) + NODE_H / 2 + PADDING,
      ],
    ]
  }, [isMobile])
  ...
  const expandedNode =
    isMobile && expandedId ? nodes.find((n) => n.id === expandedId) : null
    ...
    <div className="flex-1 flex flex-col overflow-hidden page-enter">
      ...
      {expandedNode && (
        <>
          ...
          />
          <div
            className={`course-sheet${sheetExiting ? ' course-sheet-exit' : ''}`}
          >
            <div className="w-9 h-1 rounded-sm bg-app-border mx-auto mb-[14px]" />
            <div className="text-[0.85rem] font-semibold text-app-fg text-center mb-[14px]">
              {expandedNode.data.title}
            </div>
            <div className="flex flex-wrap gap-[10px] justify-center">
              {expandedNode.data.steps.map((step, i) => (
                <StepButton
                  ...
                  onClick={() => {
                    closeSheet()
                    expandedNode.data.onStepClick(i)
                  }}
                />
              ))}
            </div>
