...
export default function LessonPage({ pyodideReady, monacoTheme }) {
  ...
  const [quizShuffles, setQuizShuffles] = useState([])
  ...
  useEffect(() => {
    ...
    loadStep(node.id, stepIdx)
      .then((step) => {
        ...
        setCurrentSection(
          location.state?.section === 'challenge' ? (hasQ ? 2 : 1) : 0
        )
    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link
            to="/course"
            className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[0.8125rem] font-medium text-app-muted no-underline transition-colors duration-150 hover:text-app-fg hover:bg-app-btn"
          >
            Back to course
          </Link>
        </div>
  ...
  const progressPct = allPassed
    ? 100
    : currentSection === 0
      ? 5
      : Math.round(5 + (currentSection / maxSection) * 80)
  const nextDisabled =
    hasQuiz && currentSection === SECTION_QUIZ && !quizAllAnswered && !allPassed
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col bg-transparent overflow-hidden page-enter">
      ...
      <div className="h-[3px] bg-app-border shrink-0 relative">
        <div
          className="absolute left-0 top-0 h-full bg-gradient-to-r from-[var(--accent-try)] to-[var(--accent-challenge)] [transition:width_0.55s_cubic-bezier(0.4,0,0.2,1)] rounded-[0_2px_2px_0] shadow-[0_0_8px_color-mix(in_srgb,var(--accent-try)_50%,transparent)]"
          style={{ width: `${progressPct}%` }}
        />
      </div>

      <div
        className="flex-1 min-h-0 overflow-y-auto bg-[radial-gradient(circle,color-mix(in_srgb,var(--text-muted)_18%,transparent)_1px,transparent_1px)] [background-size:22px_22px]"
      ...
      <LessonNav
        ...
        onNavigatePrevLesson={() =>
          navigate(`/learn/${node.id}/${stepIdx}`, {
            state: { section: 'challenge' },
          })
        }
